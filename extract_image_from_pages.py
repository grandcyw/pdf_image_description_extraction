...
for fname in doclist:
    print(f'{fname}\t')
    imgdir =os.path.join(imgpath,f'pdf{uuid.uuid4()}')   # found images are stored in this subfolder
    txtpath=os.path.join(imgdir,"imgdes.txt")
    ...
    for pno in range(page_count):
        ...
        for img in il:
            ...
            if len(imgdata) / (width * height * n) <= relsize:
                continue
            imgfile = os.path.join(imgdir, f'p{pno+1}fig{p}.{image["ext"]}')
            p=p+1
